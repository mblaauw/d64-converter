...
- Extracted 8 directory entries into `out/ikplus8/disk/files`.
- Captured live VICE RAM after 140 seconds into `snapshots/vice-capture.ram`.
- Collected 700 hardware samples into `traces/hardware-samples.json`.
- Extracted 5 screen blocks, 5 charsets, and 39 displayed sprite blocks into `assets/`.
- Game start (t0) detected at frame 6340: IRQ vector left the KERNAL default and the screen base left $0400. Earlier frames are the loader phase.
- Applied 252 joystick input events from the default autoplay script.
