...
- Extracted 8 directory entries into `out/ikplus_final/disk/files`.
- Captured live VICE RAM after 12 seconds into `snapshots/vice-capture.ram`.
- Collected 60 hardware samples into `traces/hardware-samples.json`.
- Extracted 1 screen blocks, 2 charsets, and 8 displayed sprite blocks into `assets/`.
- Game start (t0) detected at frame 7933: IRQ vector left the KERNAL default and the screen base left $0400. Earlier frames are the loader phase.
- Applied 21 joystick input events from the default autoplay script.
